...
public class SimpleApp {
  public static void main(String[] args) {
     ...
     JavaSparkContext contx = new JavaSparkContext(sparkconf);
         //int val =1; 
         //String recup="";
       JavaRDD<String> textFile = contx.textFile("onlyBayLocsUsersWithFreq.txt.save");

       JavaPairRDD<String , Integer>   counts = textFile
      .flatMap(s -> Arrays.asList(s.substring(s.indexOf(" ")+1).split(" ")).iterator())
    .mapToPair(word -> new Tuple2<>(word.substring(0,word.length()-2), Integer.parseInt(word.substring(word.length()-1)))
    .reduceByKey((a, b) -> a + b)
    .sortByKey(false);
